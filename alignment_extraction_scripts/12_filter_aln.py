...
parser.add_argument("-l", dest="len_threshold", help="Threshold length of non-gap and non-missing sites to use for filtering.", type=float, default=False);
...
if not args.percent_threshold:
  sys.exit( " * Error 3: A percent gap or missing site for filtering must be definted with -p.");

if not args.len_threshold:
  sys.exit( " * Error 4: A minimum length of non-gap and non-missing characters must be definted with -l.");
